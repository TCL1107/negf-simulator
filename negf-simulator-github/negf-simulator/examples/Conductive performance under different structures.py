...
# 先存檔，再關閉
plt.tight_layout()
plt.savefig("ML_predict_vs_real.png", dpi=300)
plt.savefig("ML_predict_vs_real.pdf")
plt.close()

# 同時把 test set 的真實值 + 預測值存成 CSV
test_results = pd.DataFrame({
    "barrier_thickness_nm": X_test["barrier_thickness_nm"].values,
    "barrier_material": X_test["barrier_material"].values,
    "interface_atom_order": X_test["interface_atom_order"].values,
    "Ef_T(E)": X_test["Ef_T(E)"].values,
    "real_I_uA": y_test.values,
    "pred_I_uA": y_pred
})
test_results.to_csv("ML_test_predictions.csv", index=False)
...
plt.savefig("ML_feature_importance.pdf")
plt.close()
