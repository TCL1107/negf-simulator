...
if __name__ == "__main__":
    ...
    # 繪 I–V
    # 繪 I–V（baseline vs dephasing）並存成圖＋CSV
plt.figure(figsize=(7,4))
plt.plot(V_list, I_base, label="baseline (η=0)")
if eta_deph > 0:
    plt.plot(
        V_list,
        I_deph,
        label=f"dephasing (η={eta_deph:g} eV, {deph_region})"
    )
...
# 檔名帶上去相干設定，避免之後搞混
tag = f"eta_{eta_deph:g}_{deph_region}"

# 存圖
plt.savefig(f"IV_compare_{tag}.png", dpi=300)
plt.savefig(f"IV_compare_{tag}.pdf")
plt.close()

# 存 baseline / dephasing 的 I–V 數據
np.savetxt(
    f"IV_baseline_{tag}.csv",
    np.column_stack([V_list, I_base]),
    delimiter=",",
    header="V(eV),I_baseline(arb.)",
    comments=""
)

if eta_deph > 0:
    np.savetxt(
        f"IV_dephasing_{tag}.csv",
        np.column_stack([V_list, I_deph]),
        delimiter=",",
        header="V(eV),I_dephasing(arb.)",
        comments=""
    )
